...
const WhyChooseUsSection = forwardRef<WhyChooseUsSectionRef, WhyChooseUsSectionProps>(({ content }, ref) => {
  ...
  const differenceColors = ['#5693C1', '#0EA5E9', '#10B981', '#F59E0B', '#8B5CF6'];
            ...
            {content.title.split(' ').map((word, i, arr) => (
              word === 'RoleReady' ? (
                <span key={i} className="relative">
                  <span className="text-[#5693C1]">{word}</span>
            ...
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                <h3 className="text-2xl font-bold text-gray-900">
                  {user?.role === 'mentor' ? 'The Mentor Advantage' : 'Our Core Differentiators'}
                </h3>
              ...
              {content.differences.map((item, index) => (
                ...
                >
                  <div
                    ...
                    style={{
                      background: `linear-gradient(135deg, ${differenceColors[index % differenceColors.length]}, ${differenceColors[index % differenceColors.length]}80)`
                    }}
                  >
                    <div className="text-white">
                      {differenceIcons[index % differenceIcons.length]}
                    </div>
          ...
          <div className="relative bg-white rounded-3xl p-8 border border-gray-100 shadow-xl hover:shadow-2xl transition-all duration-500">
            ...
            <div className="space-y-3">
              {comparisonPoints.map((item, index) => (
                <div
                  key={index}
                  className={`flex justify-between items-center p-4 rounded-xl transition-all duration-300 cursor-pointer ${activeComparison === index
                ...
                >
                  <div className="flex items-center gap-3 w-[48%]">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${activeComparison === index ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-600'
                      }`}>
                      {item.traditionalIcon}
                    </div>
                    <p className={`text-sm font-medium transition-colors ${activeComparison === index ? 'text-gray-900' : 'text-gray-600'
                      }`}>
                      {item.traditional}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center justify-center w-[4%]">
                    <ArrowRight className={`w-4 h-4 transition-all duration-300 ${activeComparison === index ? 'text-[#5693C1] scale-110' : 'text-gray-400'
                      }`} />
                  </div>

                  <div className="flex items-center gap-3 w-[48%] justify-end">
                    <p className={`text-sm font-medium text-right transition-colors ${activeComparison === index ? 'text-gray-900' : 'text-gray-700'
                      }`}>
                      {item.roleready}
                    </p>
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${activeComparison === index ? 'bg-[#5693C1]/20 text-[#5693C1]' : 'bg-gray-100 text-gray-600'
                      }`}>
                      {item.rolereadyIcon}
                    </div>
                  </div>
