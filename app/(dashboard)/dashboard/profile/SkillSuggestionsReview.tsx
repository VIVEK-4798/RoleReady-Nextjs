  ...
  const handleParseResume = async () => {
    ...
    try {
      ...
      if (apiResponse.success && apiResponse.data) {
        ...
        if (newSuggestions > 0) {
          toast.success(`🎉 Found ${newSuggestions} new skills in your resume!`);
        } else if (totalSkills > 0) {
          toast(`Found ${totalSkills} skills, but you already have them all!`, { icon: 'ℹ️' });
        } else {
          toast('No matching skills found. The skills in your resume might not be in our database yet.', { icon: 'ℹ️' });
        }
        await fetchSuggestions();
      } else {
        toast.error(apiResponse.error || 'Failed to parse resume');
      }
    } catch (error) {
  ...
  const handleConfirmSkills = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.success(`✅ Successfully added ${count} skill${count !== 1 ? 's' : ''} to your profile!`);
        setShowSuggestions(false);
        setSuggestions([]);
        ...
        toast.error(data.error || 'Failed to add skills');
      }
    } catch (error) {
      ...
      <div className="bg-white rounded-xl border-2 border-green-200 p-6 shadow-md">
        ...
        <div className="space-y-2 mb-4 max-h-96 overflow-y-auto">
          {suggestions.map((skill) => (
            <label
              key={skill.skill_id}
              className={`flex items-center gap-3 p-3 border-2 rounded-xl cursor-pointer transition-all ${selectedSkills.has(skill.skill_id)
